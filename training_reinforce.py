...
class REINFORCEAgent:
    ...
    def update_rl(self, log_probs, returns, entropies):
        ...
        # avg_baseline = np.mean(self.reward_history) if len(self.reward_history) > 0 else 0
        # # Subtract baseline from returns before standardizing
        # returns_t = torch.as_tensor(returns, dtype=torch.float32, device=self.device) - avg_baseline
...
train_pairs = prepare(dataset=CURRENT_CONFIG["dataset"], mode=CURRENT_CONFIG["mode"])
...
agent.train(epochs=EPOCHS, bc_epochs=int(EPOCHS * EXPERT_FRACTION))
